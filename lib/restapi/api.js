...
RestApi.generateCsrfToken = function(secret) {
  ...
  return this.auth.generateToken(secret);
};
 ...
 * Execute call.
 *
 * @param {string|object} method Http method or model method.
...
RestApi.execute = function(method, objName, args, cb) {
  if (typeof method === 'object') {
    ...
    objName = method.url || method.name;
    args = { data: method.data };

    if (typeof cb !== 'function' && typeof method.success === 'function') {
      var success = method.success;
      var error = (typeof method.error === 'function') ? method.error : function() {};
      cb = function(err, result) {
        if (!U.isEmpty(err)) {
          error(err);
        } else {
    ...
    method = method.type || method.method;
  }
...
function execute(api, method, cb, req, res, next) {
  ...
  if (typeof api.settings.inputTransform === 'function') {
    params = api.settings.inputTransform(req, parsedParams);
  }
  ...
  parseBody(req, function(err, body) {
    if (typeof api.settings.bodyTransform === 'function') {
      params.body = api.settings.bodyTransform(req, body, params);
    } else {
      params.body = body;
    }
...
function pipe(api, headers, req, res, next) {
  return function(err, result) {
    ...
    if (!U.isEmpty(err)) {
      ...
      if (result) {
        if (result.data) {
          if (typeof api.settings.outputTransform === 'function') {
            data = api.settings.outputTransform(req, result.data);
          } else {
            data = result.data;
          }
          ...
          if (typeof api.settings.outputTransform === 'function') {
            data = api.settings.outputTransform(req, result);
          } else {
            data = result;
          }
        }
