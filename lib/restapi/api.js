...
function isAuthorized(api, req) {
  ...
  } else if (!U.isEmpty(token)) {
    return verifyToken(api, token);
...
function verifyToken(api, token) {
  var secrets = api.settings.secrets;
...
function generateCsrf(keyType, api, req, res, next) {
  var csrf = new CSRF();
  if (keyType == "secret") {
    csrf.secret(function(err, result) {
      pipe(api, req, res, next)(err, { 'secret': result });
    });

  } else if (keyType == "token") {
    var secret = req.query['secret'];
    if (!U.isEmpty(secret)) {
      var token = csrf.create(secret);
      pipe(api, req, res, next)(0, { 'token': token });
...
function handle(api, req, res, next) {
  ...
  if (/^(\/|)--help$/i.test(url)) {
    ...
    if (match) {
      generateCsrf(match[2].toLowerCase(), api, req, res, next);
      return;
    }
