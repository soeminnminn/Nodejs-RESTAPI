  ...
  if (typeof api.settings.isAuthorized === 'function') {
    ...
    if (!U.isEmpty(api.settings.secrets) || !U.isEmpty(api.settings.csrf)) {
      if (U.isEmpty(api.settings.users)) {
        ...
        if (!U.isEmpty(api.settings.csrf.checkBasicAuth)) {
          this.checkBasicAuth = api.settings.csrf.checkBasicAuth;
        }
...
authentication.prototype.verifyBasicAuth = function(param) {
  if (this.authType === Types.BASIC_AUTH || this.authType === Types.CSRF_AUTH) {
    var result = false;
    var req = {};
    if (typeof param === 'string') {
      req.headers = { 'authorization': param };
...
authentication.prototype.verifyToken = function(param) {
  if (this.authType === Types.CSRF_AUTH) {
    ...
    if (typeof param === 'string') {
      ...
      if (this.checkBasicAuth && !this.verifyBasicAuth(param)) {
        var cookies = new COOKIES(param, {}, {});
        if (!this.verifyBasicAuth(cookies.get('authorization'))) {
          return false;
        }
      }
