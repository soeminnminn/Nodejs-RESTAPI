...
function query(api, table, includes) {
  ...
  if (!U.isEmpty(table)) {
    selectQuery(api, table)
      .then(function(result) {
        return applyRelations(api, table, includes, result);
...
function applyRelations(api, table, tables, result) {
  var deferred = Q.defer();
  if (!U.isEmpty(table.relations) && !U.isEmpty(result)) {
    var funcs = [];
    var relations = [];
    for (var j in table.relations) {
      var ref = table.relations[j].table;
      if (!U.isEmpty(tables[ref])) {
        var col = table.relations[j].column;
        var relTable = addRelationCondition(api, tables[ref], j, col, result[table.table]);
        relTable.relRef = {
          'primary': col,
          'foreign': j
        };
      ...
      .then(function(relResult) {
        if (!U.isEmpty(relResult)) {
          for (var i in relResult) {
            ...
            applyRelationsData(table, relations[i], result, relResult[i][relTable.table]);
          }
        }
        deferred.resolve(result);
...
function applyRelationsData(table, relTable, result, relResult) {
  if (!U.isEmpty(relResult)) {
    var relObj = {};
    var primary = relTable.relRef.primary;
    var foreign = relTable.relRef.foreign;
    var tableName = table.table;
    for (var i in relResult) {
      var d = relResult[i];
      relObj['#' + d[primary]] = d;
    }
    for (var i in result[tableName]) {
      var foreignKey = result[tableName][i][foreign];
      delete(result[tableName][i][foreign]);
      result[tableName][i][relTable.table] = relObj['#' + foreignKey];
    }
  }
  return result;
}
