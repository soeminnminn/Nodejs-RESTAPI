...
function execute(api, params, cb) {
  if (!U.isEmpty(params.tables)) {
    ...
    for (var i in params.tables) {
      ...
      funcs.push(queryTable(api, table, includes));
    }
...
function queryTable(api, params, includes) {
  var deferred = Q.defer();
  if (!U.isEmpty(params)) {
    //
    query(api, params).then(function(result) {
      deferred.resolve(result);
...
function query(api, table) {
  ...
  if (!U.isEmpty(table)) {
    ...
    if (!U.isEmpty(tableparams.order)) {
