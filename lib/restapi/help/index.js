...
help.render = function(basedir, req, res, next) {
  var readmeFile = PATH.join(basedir, 'Readme.md');
  if (FS.existsSync(readmeFile)) {
    var converter = new SHOWDOWN.Converter();
    converter.setFlavor('github');
    converter.setOption('tables', true);

    FS.readFile(readmeFile, 'utf8', function(err, text) {
      var data = converter.makeHtml(text);
      var headAnchor = HEAD_ANCHOR.join('');
      data = data.replace(/(<h[1-6] id=")([^\/]+)(">)([^<>]+<\/h[1-6]>)/gi, function(x, x1, x2, x3, x4) {
        var anchor = headAnchor.replace(/\{\{name\}\}/g, x2);
