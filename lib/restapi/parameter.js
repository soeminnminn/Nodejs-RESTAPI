...
function parseRelations(api, relation) {
  ...
  for (var i in val) {
    var val1 = val[i];
    if (!U.isEmpty(val1)) {
      val1 = val1.replace(/^([^\.\,]+),/, '#primary.$1,');
      val1 = val1.replace(/([^\.\,]+).([^\.\,]+)/g, '"$1": {"column": "$2"}');
      val1 = "{" + val1 + "}";
      values.push(val1);
    }
  }
...
function applyRelations(params, val) {
  ...
  for (var i in params.tables) {
    ...
    if (params.tables[i].type == 'primary') continue;
    for (var j in relations) {
      if (relations[j][table]) {
        if (!params.tables[i]['relations']) {
          params.tables[i]['relations'] = {};
        }
        var colKey = relations[j][table].column;
        if (params.tables[i].primaryKey == colKey) continue;

        params.tables[i]['relations'][colKey] = {
          table: relations[j].primary,
          column: relations[j][relations[j].primary].column
        };
      }
    }
  }
