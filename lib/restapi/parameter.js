...
function parseRelationItem(val) {
  if (val) {
    val = val.replace(/^([^\.\,]+),/, '#primary.$1,');
    val = val.replace(/([^\.\,]+).([^\.\,]+)/g, '"$1": {"column": "$2"}');
    val = "{" + val + "}";
  }
  return val;
}

function parseRelation(api, relation) {
  var val = parseParam(relation, 'a-zA-Z0-9\-_,.');
  var values = [];
  if (Array.isArray(val)) {
    for (var i in val) {
      values.push(parseRelationItem(val[i]));
    }

  } else {
    values.push(parseRelationItem(val));
  }
  return values;
}
  ...
  if (typeof query === 'object') {
    ...
    for (var i in query) {
      ...
      if (key == 'include' || key == 'exclude') {
        ...
        tempQuery[key] = parseRelation(api, val);
    ...
    if (params && params.database && params.tables) {
      ...
      for (var i in params.tables) {
        ...
        if (tempQuery['distinct']) {
          params.tables[i].distinct = tempQuery['distinct'];
        }
      ...
      if (tempQuery['relation']) {
        params['relations'] = [];
        for (var i in tempQuery['relation']) {
          var rel = tempQuery['relation'][i];
          rel = rel.replace(/#primary/g, params.table);
          params['relations'].push(JSON.parse(rel));
        }
      }
    }
  }
