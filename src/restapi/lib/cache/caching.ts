 ...
 */
export function caching(args?: any) {
  args = args || {};
  const self: any = {};
  ...
  if (typeof args.isCacheableValue === "function") {
    ...
    self._isCacheableValue = (value: any) => {
      return value !== undefined;
    };
  }

  function wrapPromise(key: any, promise: any, options?: any) {
    return new Promise((resolve, reject) => {
      self.wrap(key, function(cb: Function) {
        Promise.resolve()
        .then(promise)
        .then((result: any) => {
          cb(undefined, result);
   ...
   *   });
   */
  self.wrap = function(key: any, work: any, options?: any, cb?: Function) {
    ...
    self.store.get(key, options, (err: any, result: any) => {
      if (err && (!self.ignoreCacheErrors)) {
        ...
        work((err: any, data: any) => {
          ...
          self.store.set(key, data, options, (err: any) => {
            if (err && (!self.ignoreCacheErrors)) {
              callbackFiller.fill(key, err);
            } else {
              callbackFiller.fill(key, undefined, data);
